...
setCookie = async () => {
  ...
  try {
    ...
    await page.$eval('input[name=pwd]', (el,password) => {el.value = password}, password)
    await page.click('input[type="submit"]')
    let cookies = await page.cookies()
    cookie = `${cookies[0].name}=${cookies[0].value}`

    fs.writeFile('cookie.txt', cookie,'utf8',(err)=> {
...
getSales = async (date_start, date_end, product) => {
  ...
  try {
    ...
    await (async () => {
      try {
        let file = await fs.readFile("cookie.txt", "utf-8");
        cookie = file
      } catch (e) {
        console.log("e", e);
      }
    ...
    const config = {
      method: 'post',
      url: 'https://www.primepharmacy.gr/eshop/reporting/products_timespans.htm',
      headers: {
        'Cookie': cookie,
    ...
    try{
    const result = dom.window.document.querySelector('#tblDataReport > tbody > tr > td > strong ').textContent
