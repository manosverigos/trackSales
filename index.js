  ...
  try {
    ...
    for (offer of resultSales){
      if(!offer.completed){
        ...
        const updatedOffer = await collection.updateOne({_id:o_id}, {$set: {current_sales: await sales}})

        if(sales >= parseInt(offer.sales_num)) {
          const overdueOffer = await collection.updateOne({_id:o_id}, {$set: {overdue: true}})
        }
        if((parseInt(offer.sales_num) - sales <= 2) && !(sales >= offer.sales_num) && (offer.warning != 'done') ) {
          const warningOffer = await collection.updateOne({_id:o_id}, {$set: {warning: 'true'}})
        }
        if(parseInt(offer.sales_num) - sales > 2) {
          const notOverdueOffer = await collection.updateOne({_id:o_id}, {$set: {overdue: false, warning: 'false'}})
        }
      }
    }
    ...
    for(offer of resultTime){
      ...
      if(!offer.completed){
        ...
        if((dayDiff(today, offer.endDate) >= -2) && (dayDiff(today, offer.endDate) <0) && (offer.warning != 'done')){
          const warningOffer = await collection.updateOne({_id:o_id}, {$set: {warning: 'true'}})
        }
